...
import "./TestBase721.sol";
import "../PatchFragment.sol";
import "@patchwork/PatchworkProtocol.sol";
import "@patchwork/PatchworkProtocolAssigner.sol";

contract PatchFragmentTest is Test {
    PatchworkProtocol _prot;
    TestBase721 _base721;
    PatchFragment _myContract;
    ...
    function setUp() public {
        ...
        _base721 = new TestBase721();
        _myContract = new PatchFragment(address(_prot), _scopeOwner);
    ...
    function testSupportsInterface() public {
        ...
        assertTrue(_myContract.supportsInterface(type(IPatchworkPatch).interfaceId));
        assertTrue(_myContract.supportsInterface(type(IPatchworkSingleAssignable).interfaceId));
    }
    ...
    function testPackUnpackMetadata() public {
        PatchFragment.Metadata memory originalData;
        ...
        PatchFragment.Metadata memory unpackedData = _myContract.unpackMetadata(packed);
    ...
    function testName() public {
        vm.startPrank(_scopeOwner);
        uint256 baseTokenId = _base721.mint(_userAddress);
        uint256 tokenId = _myContract.mintPatch(_userAddress, IPatchworkPatch.PatchTarget(address(_base721), baseTokenId));
