...
function isPointInDrop(
    ...
    // 1. Find center of the circle from its bounding rect
    const circleCenter = {
        x: drop.left + drop.width / 2,
        y: drop.top + drop.height / 2,
    };

    // 2. Get distance from point to circle center using Pythagorean theorem
    const distance = Math.sqrt(
        Math.pow(point.x - circleCenter.x, 2) +
            Math.pow(point.y - circleCenter.y, 2),
    );

    // 3. Circle radius is half the width (assuming circle is a square)
    const radius = drop.width / 2;

    // 4. Point is inside if distance to center is less than radius
