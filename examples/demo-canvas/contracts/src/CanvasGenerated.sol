...
abstract contract CanvasGenerated is Patchwork721, PatchworkLiteRef, IPatchworkMintable {
    ...
    constructor(address _manager, address _owner)
        Patchwork721("canvas", "Canvas", "CANVAS", _manager, _owner)
        PatchworkLiteRef()
    {}

    function schemaURI() pure external override returns (string memory) {
        return "https://canvas.patchwork.dev/schemas/canvas.json";
    }

    function imageURI(uint256 tokenId) pure external override returns (string memory) {
        return string.concat("https://canvas.patchwork.dev/assets/canvas/", Strings.toString(tokenId), "");
    }
