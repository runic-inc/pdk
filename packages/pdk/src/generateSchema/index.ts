...
export async function generateSchema(configPath: string) {
    try {
        ...
        Object.entries(projectConfig.contracts).forEach(
            ([contractName, contractConfig]) => {
                ...
                const fields = [
                    ...
                    {
                        key: "mintTxId",
                        value: "p.string().references('Tx.id')",
                    },
                    {
                        key: "burnTxId",
                        value: "p.string().references('Tx.id').optional()",
                    },
                    { key: "mintTx", value: "p.one('mintTxId')" },
                    { key: "burnTx", value: "p.one('burnTxId')" },
                    {
                        key: "contractId",
                        value: "p.string().references('Contract.id')",
                    },
                ];
                ...
                contractConfig.fields.forEach((field) => {
                    if (field.type === "literef" || field.arrayLength) {
                        ...
                        fields.push({
                            key: field.key,
                            value: `p.many('${refTableName}.${camelCase(contractName)}Id')`,
                        });
                        const refFields = [
                            { key: "id", value: "p.string()" },
                            {
                                key: `${camelCase(contractName)}Id`,
                                value: `p.string().references('${contractName}.id')`,
                            },
                            { key: "value", value: getFieldType(field.type) },
                            { key: "timestamp", value: "p.bigint()" },
                        ];
                        additionalTables.push(
                            createTableFromObject(refTableName, refFields)
                        );
                    } else {
                        fields.push({
                            key: field.key,
                            value: `${getFieldType(field.type)}.optional()`,
                        });
                    }
                });
                ...
                if (fragmentRelationships[contractName]) {
                    fragmentRelationships[contractName].forEach((relation) => {
                        fields.push({
                            key: camelCase(`${relation}Id`),
                            value: `p.string().references('${relation}.id').optional()`,
                        });
                    });
                }

                if (
                    projectConfig.contractRelations[contractName]?.fragments
                        .length > 0
                ) {
                    projectConfig.contractRelations[
                        contractName
                    ].fragments.forEach((fragment) => {
                        fields.push({
                            key: camelCase(`${fragment}`),
                            value: `p.many('${fragment}.${camelCase(contractName)}Id')`,
                        });
                    });
                }

                fields.push({ key: "timestamp", value: "p.bigint()" });
                contractDBEntities.push(
                    createTableFromObject(contractName, fields)
                );
            }
        );

        await createSchemaFile(
            [...generalDB, ...contractDBEntities, ...additionalTables],
            ponderSchema
        );
