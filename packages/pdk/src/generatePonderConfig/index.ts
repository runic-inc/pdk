...
export async function generatePonderConfig(configPath: string) {
    try {
        ...
        try {
            ...
            console.error(
                `Make sure the ABI directory exists relative to the config file.`
            );
            return;
        }
        ...
        if (!projectConfig.networks) {
            console.error(
                `No networks found in the project config. Cannot build network configuration.`
            );
            return;
        }
        ...
        const contracts = Object.entries(projectConfig.contracts)
            .map(([contractName, contractConfig]) => {
                ...
                if (!projectConfig.deployments || !projectConfig.networks) {
                    console.error(
                        `No deployments or networks found. Cannot build contract config for ${contractName}`
                    );
                    return "";
                }
        ...
        const networks = Object.entries(projectConfig.networks).map(
            ([networkName, network]) => {
        ...
        console.log(
            `Ponder config generated successfully: ${ponderConfigPath}`
        );
    } catch (error) {
...
function contractNetworkTemplate(
    name: string,
    networkName: string,
    deployments: Deployment<string>[],
    network: Network
