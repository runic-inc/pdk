...
abstract contract SecondContractGenerated is PatchworkPatch {
    ...
    constructor(address _manager, address _owner)
        Patchwork721("test", "SecondContract", "SC", _manager, _owner)
    {}

    function schemaURI() pure external override returns (string memory) {
        return "https://mysecondthing/my-metadata.json";
    }

    function imageURI(uint256 tokenId) pure external override returns (string memory) {
        return string.concat("https://mysecondthing/my/", Strings.toString(tokenId), ".png");
    }

    function _baseURI() internal pure virtual override returns (string memory) {
        return "https://mysecondthing/my/";
    }
    ...
    function schema() pure external override returns (MetadataSchema memory) {
        ...
        entries[0] = MetadataSchemaEntry(1, 0, FieldType.CHAR32, 1, FieldVisibility.PUBLIC, 0, 0, "description");
    ...
    function packMetadata(Metadata memory data) public pure returns (uint256[] memory slots) {
        slots = new uint256[](1);
        slots[0] = PatchworkUtils.strToUint256(data.description);
        return slots;
    }

    function unpackMetadata(uint256[] memory slots) public pure returns (Metadata memory data) {
        uint256 slot = slots[0];
        data.description = PatchworkUtils.toString32(slot);
        return data;
    }

    // Load Only description
    function loadDescription(uint256 tokenId) public view returns (string memory) {
        uint256 value = uint256(_metadataStorage[tokenId][0]);
        return PatchworkUtils.toString32(value);
    }

    // Store Only description
    function storeDescription(uint256 tokenId, string memory description) public {
        if (!_checkTokenWriteAuth(tokenId)) {
            revert IPatchworkProtocol.NotAuthorized(msg.sender);
        }
        _metadataStorage[tokenId][0] = PatchworkUtils.strToUint256(description);
    }
}